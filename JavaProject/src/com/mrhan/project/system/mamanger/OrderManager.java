			...
			public void select(FlowView last, FlowView now) {
				...
				try{
					...
					for(GoodsInfo gi : allg){
						List<OrderGoodsInfo> all=ogid.getOrderGoodsByGoods(gi);//获取当前商品的订单数量
					    int num=0;
					    int size=all.size();
					    for(OrderGoodsInfo ogi :all ){
					    	num+=ogi.getNum();
					    }
						if(size>0){
						out(1,"\t");out(gi.getGoodId());
						out(2,"\t");out(gi.getGoodsType().getName());
						out(2,"\t");out(gi.getName());
						out(2,"\t");out(gi.getPrice());
						out(2,"\t");out(gi.getDiscount());
						out(2,"\t");out(num);
						out(2,"\t");outln(size);
					  }
					}
				...
				try{
					...
					if(oi!=null){
						...
						for(OrderGoodsInfo ogi : allgos){
							...
							out(2,"\t");out(ogi.getGoodsInfo().getGoodsType().getName());
							out(2,"\t");out(ogi.getGoodsInfo().getName());
							out(2,"\t");out(ogi.getGoodsInfo().getPrice());
							
							out(2,"\t");out(ogi.getGoodsInfo().getDiscount());
							out(2,"\t");outln(ogi.getNum());
						}
					}else{
